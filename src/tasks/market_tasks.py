...
class MarketTasks:
    ...
    def create_research_task(self, symbol: str, agent) -> Task:
        ...
        return Task(
            ...
            3. CONTEXTO MACROECONÔMICO BRASILEIRO:
               • Taxa SELIC atual e expectativas futuras
               • IPCA (inflação) e impacto no setor
               • Risco Brasil (CDS 5 anos) e percepção internacional
               • Cotação USD/BRL e impacto na empresa
               • Política monetária do Banco Central
               • Fluxo de capital estrangeiro na B3
               • Volume de entrada/saída de investimento externo
               • PIB brasileiro e tendências econômicas
            ...
            • Taxa SELIC atual: X,XX% (tendência e impacto)
            • IPCA (inflação): X,XX% (efeito no setor/empresa)
            • Risco Brasil: XXX pontos base (percepção internacional)
            • USD/BRL: R$ X,XX (impacto cambial na empresa)
            • Fluxo de capital: Entrada/saída na B3
            • PIB e perspectivas econômicas nacionais
    ...
    def create_fundamental_analysis_task(self, symbol: str, agent, research_task: Task) -> Task:
        ...
        return Task(
            ...
            2. INDICADORES FUNDAMENTALISTAS:
               ...
               • Análise vs índices brasileiros (IBOV, IBRX, etc.)
               • Impacto da taxa SELIC na avaliação
               • Efeito da inflação (IPCA) nos múltiplos
            ...
            • **Pontuação quantitativa: X.XX (0.00 a 1.00)**
              - 0.00-0.20: Venda Forte
              - 0.21-0.40: Venda
              - 0.41-0.60: Neutro/Mantenha
              - 0.61-0.80: Compra
              - 0.81-1.00: Compra Forte
    ...
    def create_technical_analysis_task(self, symbol: str, agent, research_task: Task) -> Task:
        ...
        return Task(
            ...
            6. MOMENTUM E FORÇA RELATIVA:
               ...
               • Força relativa vs IBOVESPA e IBRX-100
               • Desempenho vs pares setoriais
               • Sinais de sobrecompra/sobrevenda
               • Divergências de momentum
               • Impacto do fluxo de capital estrangeiro na B3
            ...
            • **Pontuação técnica geral: X.XX (0.00 a 1.00)**
              - 0.00-0.20: Muito Baixista
              - 0.21-0.40: Baixista  
              - 0.41-0.60: Neutro
              - 0.61-0.80: Altista
              - 0.81-1.00: Muito Altista
    ...
    def create_daytrader_task(self, symbol: str, agent, research_task: Task, technical_task: Task) -> Task:
        ...
        return Task(
            ...
            📊 PONTUAÇÃO DAYTRADE (OBRIGATÓRIA)
            **Recomendação numérica para próxima abertura: X.XX**
            
            **Escala de Pontuação:**
            • 0.00-0.20: VENDA FORTE (short agressivo)
            • 0.21-0.40: VENDA (short ou fora do mercado)
            • 0.41-0.60: NEUTRO (aguardar setup melhor)
            • 0.61-0.80: COMPRA (long moderado)
            • 0.81-1.00: COMPRA FORTE (long agressivo)
            
            **Esta pontuação deve ser baseada em:**
            - Confluência técnica (peso 40%)
            - Momentum intraday (peso 30%)
            - Volume e fluxo (peso 20%)
            - Contexto de mercado (peso 10%)
    ...
    def create_investment_recommendation_task(self, symbol: str, agent, all_previous_tasks: list) -> Task:
        ...
        return Task(
            ...
            • **PONTUAÇÃO FINAL: X.XX (0.00 a 1.00) - OBRIGATÓRIA**
            ...
            **Metodologia de Cálculo (transparente):**
            • Análise Fundamentalista: X.XX (peso 40%)
            • Análise Técnica: X.XX (peso 30%)  
            • Day Trade Score: X.XX (peso 20%)
            • Contexto Macro BR: X.XX (peso 10%)
            
            **Escala Final Consolidada:**
            • 0.00-0.20: VENDA FORTE (recomendação veemente de saída)
            • 0.21-0.40: VENDA (recomendação de saída gradual)
            • 0.41-0.60: NEUTRO/MANTENHA (sem ação recomendada)
            • 0.61-0.80: COMPRA (recomendação de entrada)
            • 0.81-1.00: COMPRA FORTE (recomendação veemente de entrada)
